...
// const updatedPeopleFather = people.map(() => {});

// ! Задание 3

const malePeople = people.filter((human) => {
  return human.sex === "m";
});
console.log(malePeople);
const femalePeople = people.filter((human) => {
  return human.sex === "f";
});
console.log(femalePeople);

const peoplePrint = document.querySelector("p");
const malePeopleBtn = document.querySelector(".male");
malePeopleBtn.addEventListener("click", findMalePeople);
const femalePeopleBtn = document.querySelector(".female");
femalePeopleBtn.addEventListener("click", findFemalePeople);

function findMalePeople() {
  peoplePrint.textContent = malePeople.map((person) => person.name).join(", ");
}

function findFemalePeople() {
  peoplePrint.textContent = femalePeople
